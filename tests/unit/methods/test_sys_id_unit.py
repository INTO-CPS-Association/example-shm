...
from methods.setup_data import setup_client, get_data
...
def test_sysid_returns_expected_keys(sample_data, sysid_params):
    ...
    expected_keys = {'Fn_poles', 'Fn_poles_std', 'Xi_poles', 'Xi_poles_std', 'Phi_poles','Phi_poles_std','Ufx_list'}
...
def test_get_oma_results_success():
    fs = 100
    samples_needed = 600
    ...
    result, ts = get_data(600, mock_aligner)

    assert result is not None
    assert "Fn_poles" in result

def test_get_oma_results_no_data():
    fs = 100
    mock_aligner = MagicMock()
    mock_aligner.extract.return_value = (np.empty((0, 3)), datetime.now())

    result, ts = get_data(6000, mock_aligner)

    assert result is None
    assert ts is None


def test_get_oma_results_not_enough_samples():
    fs = 100
    mock_aligner = MagicMock()
    data = np.random.randn(100, 3)
    mock_aligner.extract.return_value = (data, datetime.now())

    result, ts = get_data(1000, mock_aligner)  # ask for too many samples
...
def test_get_oma_results_sysid_failure(mocker):
    ...
    result, ts = get_data(1, mock_aligner)
...
def test_publish_oma_results_retries_and_publishes_once(mocker):
    ...
    dummy_result = {
        ...
        'Fn_poles_std': np.array([[0.1, 0.1], [0.1, 0.1]]),
        'Xi_poles': np.array([[0.02, 0.03], [0.04, 0.05]]),
        'Xi_poles_std': np.array([[0.001, 0.001], [0.001, 0.001]]),
