...
def SSI(
    ...
    print("cond of hank",np.linalg.cond(H))
...
def SSI_fast(
    ...
    # print(f'{H[0,10]:.32f}')
    #print("cond of hank",np.linalg.cond(H))
    #H = np.loadtxt('C:/matlab_folder/H_PC.txt',delimiter=',')
    U1, SIG, V1_t = np.linalg.svd(H)
    #np.savetxt('C:/swarup/demo_cantilever_beam/SHM_paper_results/Full2/data/SIG_AU.txt', SIG,delimiter=',')
    #print(SIG)
    # quit()
    # print(U1)
    # print(SIG)
    # print(SIG[0],SIG[-1])
    # print("Spectral condition number",SIG[0]/SIG[-1,])
    # quit()
    Uom = U1[:, :ordmax]
    Vom = V1_t[:, :ordmax]
    Som = SIG[:ordmax]
