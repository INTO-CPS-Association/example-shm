...
def eval_yafem_model(pars=None):
    ...
    #model_pars['dofs_f'] = np.array([[8,1]])
    model_pars['dofs_f'] = np.array([[8,1]])
    model_pars['dt'] = 0.004
    N = 40
    t = np.arange(0,N,model_pars['dt'])
    model_pars['step'] = len(t)

    load = 1 * np.ones_like(t) #Unit load

    #Start wave
    load = np.zeros_like(t)
    load[0:2000]  = np.sin((15*2*np.pi)*t[0:2000])

    #Double wave
    #load = 1 * np.cos((23*2*np.pi)*t) + 1 * np.cos((6*2*np.pi)*t)

    # import scipy
    # load = np.zeros_like(t)
    # load[0:5000] = scipy.signal.chirp(t[0:5000],1,t[5000],85,"linear")

    # p = np.poly1d([5,0])
    #load = np.zeros_like(t)
    #load[0:5000] = scipy.signal.sweep_poly(t[0:5000],p)

    

    # import matplotlib.pyplot as plt
    # plt.plot(t, load, 'r', label='f(t)')
    # plt.show()
