...
from functions.plot_sysid import (plot_pre_stabilization_diagram, plot_stabilization_diagram, plot_clusters)
from functions.clustering import (cluster_func)
from functions.plot_mode_tracking import plot_tracked_modes
# from functions.mode_tracking import (cluster_tracking)
from functions.mode_tracking_functions.mode_tracking_func import cluster_tracking
...
def model_updating_run(data,Params,model_pars,tracked_clusters,fig_axes,plot=False,model_updating=False):
    ...
    (fig_ax1,fig_ax2,fig_ax3,fig_ax4) = fig_axes

    sysid_output = sysid(data, Params)

    # Stabilization graph
    if plot==True:
        fig_ax2 = plot_stabilization_diagram(oma_results=sysid_output,oma_params=Params,fig_ax=fig_ax2)
        plt.show(block=False)
        sys.stdout.flush()

    # Cluster generation
    cluster_dict_before, cluster_dict_allignment, cluster_dict = cluster_func(sysid_output, Params)
    ...
    if plot==True:
        pass
    fig_ax4 = plot_tracked_modes(tracked_clusters,Params,x_length=None,fig_ax = fig_ax4)
    plt.show(block=False)
    sys.stdout.flush()
    ...
    if model_updating is True:
        try:
            ...
            omegaMU, phi, PhiMU, myModel, _ = beam_new.eval_yafem_model(model_pars)
        ...
        if X is not None:
            return [cluster_dict, tracked_clusters,(fig_ax1, fig_ax2, fig_ax3,fig_ax4), X, omegaMU, model_pars]
        else:
            return [cluster_dict, tracked_clusters,(fig_ax1, fig_ax2, fig_ax3,fig_ax4)]
    else:
        return [cluster_dict, tracked_clusters,(fig_ax1, fig_ax2, fig_ax3,fig_ax4)]
