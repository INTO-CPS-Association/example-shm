...
from functions.clustering import (remove_complex_conjugates,remove_highly_uncertain_points)
...
def plot_pre_stabilization_diagram(
    ...
    frequencies, damping_ratios, cov_freq, cov_damping, _ = remove_complex_conjugates(oma_results)
...
def plot_stabilization_diagram_for_paper(
    ...
    if fig_ax is None:
        plt.ion()
        fig, (ax1) = plt.subplots(1,1,figsize=(7, 4), tight_layout=True)
    else:
        fig, (ax1) = fig_ax
        ax1.clear()
    ...
    if fig_ax is None:
        plt.ion()
        fig, (ax2) = plt.subplots(1,1,figsize=(7, 4), tight_layout=True)
    else:
        fig, (ax2) = fig_ax
        ax2.clear()
    ax2.set_ylabel("Damping ratio", fontsize=20, color = 'black')
...
def plot_clusters_for_paper(clusters: Dict[str,dict],
    ...
    #Pre-clean
    frequencies, _, damping_ratios, _, _ = remove_highly_uncertain_points(oma_results,oma_params)
    ...
    ax1.grid(which='major', color='gray', linestyle='-', linewidth=0.5)
    ax1.grid(which='minor', color='lightgray', linestyle='--', linewidth=0.3)
    ax1.legend(prop={'size': 16}) #bbox_to_anchor=(0.1, 1.1)
...
#     indices_frequency = frequency_coefficient_variation > oma_params['freq_threshold_coef_var']
#     indices_damping   = damping_coefficient_variation > oma_params['damp_threshold_coef_var']
...
# def plot_clusters(clusters: np.ndarray,
#         oma_results: Dict[str, Any],
#         oma_params: Dict[str, Any],
#         fig_ax)-> Tuple[matplotlib.figure.Figure, Tuple[plt.Axes,plt.Axes]]:
...
# def plot_clusters_for_paper(clusters: np.ndarray,
#         oma_results: Dict[str, Any],
#         oma_params: Dict[str, Any],
#         fig_ax)-> Tuple[matplotlib.figure.Figure, Tuple[plt.Axes,plt.Axes]]:
