...
from methods.fatigue_block import run_fatigue, fatigue as fat


def fatigue_plots(path="data"):
    data = np.load(path+'/stress_estimation.npy')

    # plt.plot(data[30])
    # plt.show()
    # print(len(data))
    # for ii, batch in enumerate(data):
    #     if ii > 2:
    #         pass
    #     elif ii == 0:
    #         print(ii)
    ...
    #     else:
    #         print(ii)
    #         stress = np.append(stress,batch,axis=0)
    # print(len(batch))
    # plt.plot(stress)
    # plt.show()
    # quit()
    ...
    beam = run_fatigue(SN_curve)

    rul_prev = 0
    for counter, batch in enumerate(data):
            print(f"count: {counter+1}")

            if np.max(batch)>1000:
                print("Maximum stress is above 10000 MPa",np.max(batch))
                print("Skipping this batch", counter+1)
                #batch = (np.array(batch)/100)
                # breakpoint()
                continue
            ...
            try:
                if counter == 0:
                    ...
                    fig6 = fat.plot_eol_rul(beam.result,start_time,end_time,output_time_unit="years",damage_sum = damage_sum)
                    ...
                    _ = fat.plot_eol_rul(beam.result,start_time,end_time,output_time_unit="years",damage_sum = damage_sum,figure=fig6)
                    #fig7, ax7, _ = fat.plot_histogram(result=beam.result,bin_width=10,hist_data={},static_mean=0,figure=fig7) #flag
                plt.pause(0.1)
                          
                time_passed = end_time - start_time
                EOF, RUL = fat.EOF_RUL(beam.result,time_passed,output_time_unit="years",damage_sum=damage_sum)
                if rul_prev-RUL > 0.25*10**6:
                    fig, ax1 = plt.subplots(1,1,figsize=(6, 4), tight_layout=True)
                    plt.plot(beam.result["stress_res"])
            ...
            print(f"EOF and RUL: {fat.EOF_RUL(beam.result,time_passed=passed,output_time_unit='years',damage_sum = damage_sum)}")
