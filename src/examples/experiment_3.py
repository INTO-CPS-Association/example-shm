...
def run_experiment_3_print(config_path):
    ...
    oma_params = {
        "Fs": fs,
        "block_shift": 15, #Should this be in methods/constants?
        "model_order": 10  #Should this be in methods/constants?
    }
    aligner_time = None
    while aligner_time is None:
        results, aligner_time = sysID.get_oma_results(aligner,oma_params)
    print(f"\n System Frequencies \n {results['Fn_poles']}")
...
def run_experiment_3_publish(config_path):
    ...
    sysID.publish_oma_results(aligner, oma_params, publish_client
