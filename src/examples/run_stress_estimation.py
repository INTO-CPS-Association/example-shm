...
def run_stress_and_strain_estimation_beam(config_path):
    aligner, data_client, sysid_config, fs = setup_sysid(config_path)
    number_of_samples = sysid_config['SamplesToCollect']
    displacement, _ = virtual_sensing(number_of_samples, aligner, data_client, fs)
...
def run_stress_estimation_and_plot(config_path):
    aligner, data_client, sysid_config, fs = setup_sysid(config_path)
    number_of_samples = sysid_config['SamplesToCollect']
    displacement, _, model_parameters = virtual_sensing(number_of_samples, aligner, data_client, fs)
    stress, _ = stress_estimation_for_beam(displacement,model_parameters)
