...
def match_cluster_to_tracked_cluster(cluster_dict: Dict[str,Any], tracked_clusters: Dict[str,Any],
    ...
    for idx, key in enumerate(cluster_dict): #Go through all clusters
        ...
        for key_t in tracked_clusters: #Go through all tracked clusters.
            ...
            if key_t == 'iteration':
                ...
                for ii in range(n_last_tracked_clusters):
                    ...
                    intervals = np.array([tracked_cluster['median_f']-tracked_cluster['global_std'][0,0], tracked_cluster['median_f']+tracked_cluster['global_std'][0,0]])
                    if freq_intervals is None:
                        freq_intervals = intervals
                    else:
                        freq_intervals = np.vstack((freq_intervals,intervals))

                    # hypothesis_test(cluster,tracked_cluster)
        ...
        print((cluster['median_f'] - greatest_interval[:,0]))
        upper_mask = (cluster['median_f'] - greatest_interval[:,1]) < 0
        print((cluster['median_f'] - greatest_interval[:,1]))
        print(lower_mask)
        print(upper_mask)
        
        match_indicies = np.argwhere(lower_mask==upper_mask)
        print("indices",match_indicies)
        # breakpoint()
...
def hypothesis_test(cluster,tracked_cluster, fig_ax1, fig_ax2, fig_ax3, fig_ax4):
    ...
    print("t0:",t0)
    if abs(t0) < abs(t):
        print("Hypothesis test is accepted. Cluster is a member of tracked cluster")
    else:
        print("Hypothesis test is discarded. Cluster is not a member of tracked cluster")
    ...
    return fig_ax1, fig_ax2, fig_ax3, fig_ax4
