...
from src.methods.sysid_functions.clean_sysid_output import (extract_from_sysid,remove_highly_uncertain_points)
...
def plot_pre_stabilization_diagram(
    ...
    ax1 = add_scatter_data(ax1,x,y_model_order,std_freq,error_dir="h")
    ...
    ax2 = add_scatter_data(ax2,x,y,std_damping,error_dir="v")
...
def plot_stabilization_diagram(
    ...
    (frequencies, std_freq, damping_ratios,
     std_damping, _, __, ___) = remove_highly_uncertain_points(sysid_results,sysid_params)

    x = frequencies.flatten(order="f")
    y_model_order = np.array([i // len(frequencies) for i in range(len(x))]) * 1

    std_bound = sysid_params['bound_multiplier']
    ax1 = add_scatter_data(ax1,x,y_model_order,std_freq,error_dir="h",bound=std_bound)
    ax1 = add_plot_standard_flair(ax1,sysid_params)
    ...
    ax2 = add_scatter_data(ax2,x,y,std_damping,error_dir="v",bound=std_bound)
    ax2 = add_plot_annotation(ax2,x,y,y_model_order)
    ax2 = add_plot_standard_flair(ax2,sysid_params)

    ax2.set_title("Damping ratios")
    ax2.set_ylabel("Damping ratio", fontsize=20, color = 'black')
    ax2.set_ylim(0, max(y[~np.isnan(y)]+std_damping.flatten(order="f")[~np.isnan(y)])+0.005)
...
def add_scatter_data(ax: matplotlib.axes.Axes, x: np.ndarray[float], y: np.ndarray[float],
                     std: np.ndarray[float], error_dir: str = "h", mark: str ="o",
                     lab: str ='Non clustered',size: float = 50, bound: int = 2) -> matplotlib.axes.Axes:
    """
    ...
    Args:
        ...
        std (np.ndarray[float]): standard deviation for errorbars
        ...
        bound (int): Multiplier for standard deviation
    ...
    if std is not None:
        xerr = std.flatten(order="f") * bound
        if error_dir == "h":
