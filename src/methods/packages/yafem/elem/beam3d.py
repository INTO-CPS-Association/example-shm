...
from methods.packages.yafem.elem.beam3d_circ_func import *
from methods.packages.yafem.elem.beam3d_gen_func import *
...
class beam3d(core_elem):
    ...
    def __init__(self, my_nodes, pars):
        ...
        # self.G = np.kron(np.eye(4), self.T)
        ...
        self.Nl[np.ix_([1], indby)] += Nby.squeeze() # lateral displacement y
        self.Nl[np.ix_([2], indbz)] += Nbz.squeeze() # lateral displacement z

        # Strain interpolation matrix in the mid point
        self.Bl[np.ix_([0], inda)]  += Ba.squeeze()  # axial strain
        self.Bl[np.ix_([1], indby)] += Bby.squeeze() # curvature in y
        self.Bl[np.ix_([2], indbz)] += Bbz.squeeze() # curvature in z
