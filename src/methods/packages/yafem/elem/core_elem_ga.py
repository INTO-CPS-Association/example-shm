...
class core_elem:
    ...
    def extract_pars_core(self,pars):
        ...
        self.nodal_labels = pars.get("nodal_labels", [1])
    ...
    #%% compute the mass matrix (it is assumed that it is defined at the initialization of the element)
    def compute_M(self):
        return self.M
        
    #%% compute the damping matrix (it is assumed that it is defined at the initialization of the element)
    def compute_C(self,u,v,q,t,i):
        return self.C

    #%% compute the stiffness matrix (it is assumed that it is defined at the initialization of the element)
    def compute_K(self,u,v,q,t,i):
        return self.K      
    
    #%% compute the restoring force (it is assumed that the element is linear elastic)
    ...
    def compute_Zq(self,dofs_q):
        ...
        if jnp.shape(dofs_q)[0] != 0:
            rows, cols, found, all_true = core_elem.compute_collocations(self.dofs_q, dofs_q)
