...
from methods.packages.yafem.nodes import *
#from yafem.elem.core_elem import *
from core_elem_ga import core_elem # get customized version (GA)
#from yafem.model import *
from model_ga import model # get customized version (GA)

class simulation:

    # #pars : Dict = None
    # recorder_tags : typing.List[str] = []
    # recorder_fields : typing.List[np.ndarray[np.float64]] = []
    # M : np.ndarray[np.float64] = None
    # Cp : np.ndarray[np.float64] = None
    # t : np.ndarray[np.float64] = None
    # u : np.ndarray[np.float64] = None
    # v : np.ndarray[np.float64] = None
    # a : np.ndarray[np.float64] = None
    # r : np.ndarray[np.float64] = None
    # l : np.ndarray[np.float64] = None
    # gamma : float = None
    # beta : float = None
    # my_model : model = None

    # pre-loading a message for simulations
    msg = "step {} converged in {} iterations with residual {}"
    ...
    def __init__(self,my_model,pars={}):
        ...
        # each field is a numpy array with recorder_tag as filename
        ...
        # self.M = self.my_model.compute_M()
        self.M = self.my_model.M

        # initialize the proportional damping matrix (constant)
        # self.Cp = self.my_model.compute_Cp()
        self.Cp = self.my_model.Cp

        # initialize the time axis
        # self.t = np.arange(1, self.my_model.step * self.my_model.dt + self.my_model.dt, self.my_model.dt)
    ...
    def dynamic_analysis(self, step1=0, step2=None,output=True):
        ...
        for i in range(step1,step2-1):
            ...
            self.u[:,i+1],self.v[:,i+1],self.a[:,i+1],self.r[:,i+1] = self.dynamic_analysis_step(self.u[:,i],self.v[:,i],self.a[:,i],self.t[i],i,output)
    ...
    # perform dynamic analysis step
    def dynamic_analysis_step(self,u0,v0,a0,t0,i0,output):
        ...
        while True:
            ...
            tol = np.linalg.norm(res) / self.my_model.ndof / (1 + np.linalg.norm(np.dot(self.my_model.Bf.todense(), self.my_model.g_f[:, i1])) / self.my_model.ndof)
            ...
            # # check convergence
            # if cc > self.iter_max or np.linalg.norm(res)/(1+np.linalg.norm(self.my_model.Bf @ self.my_model.g_f[:,i1])) < self.tol_min:
            #     print('step ' + str(i1) + ' converged in ' + str(cc+1) + ' iterations' + ' with residual ' + str(np.linalg.norm(res)/(1+np.linalg.norm(self.my_model.Bf @ self.my_model.g_f[:,i1]))))
            #     break
            # else:
            #     cc += 1
    ...
    def static_analysis(self, step1=0, step2=None,output=True):
            ...
            for i in range(step1, step2-1):
                ...
                self.u[:, i+1], self.l[:, i+1], self.r[:, i+1] = self.static_analysis_step(self.u[:, i], self.l[:, i], self.t[i], i,output)
    ...
    def static_analysis_step(self, u0, l0, t0, i0,output):
        ...
        while True:
            ...
            if self.my_model.Bu.shape[1] != 0 and self.my_model.Bu.nnz != 0:
                dl = -spsolve(csr_array(np.atleast_2d(self.my_model.Bu.T @ spsolve(K, self.my_model.Bu))), self.my_model.Bu.T @ (u1 + duI) - self.my_model.g_u[:, i1])
            ...
            # Convergence tolerance
            tol = np.linalg.norm(res) / self.my_model.ndof / (1 + np.linalg.norm(self.my_model.Bf @ self.my_model.g_f[:, i1] + self.my_model.Bu @ l1) / self.my_model.ndof)
    ...
    def arclen_analysis(self, step1=0, step2=None, output=True):
        ...
        for i in range(step1, step2-1):
            ...
            # Single analysis step
            self.u[:, i+1], self.l[:, i+1], self.r[:, i+1] = self.arclen_analysis_step(self.u[:, i], self.l[:, i], self.t[i], i, output)

        return self.u, self.l, self.r

    def arclen_analysis_step(self, u0, l0, t0, i0, output):
        ...
        while True:
            ...
            dl1, dl2 = self.arclen_analysis_dll(duI, duII, Du, Dl, ds)
    ...
    def arclen_analysis_dll(self, duI, duII, Du, Dl, ds):
        ...
        if discriminant > 0:
            dl1 = (-c2 + np.sqrt(discriminant)) / (2 * c1)
            dl2 = (-c2 - np.sqrt(discriminant)) / (2 * c1)
        else:
            dl1 = -c2 / (2 * c1)
            dl2 = -c2 / (2 * c1)
    ...
    def dump_to_model(self,step=1):
        
        # simulation results to the model
