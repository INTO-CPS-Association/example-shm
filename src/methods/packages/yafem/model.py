...
#%% compute the collocation matrix for controlled forces
    def compute_Bf(self):
        ...
        self.Bf = coo_array((np.ones_like(rows, dtype=bool), (rows, cols)),shape=(dofs.shape[0], dofs_f.shape[0])).tocsc()
...
#%% compute the collocation matrix for controlled displacements
    def compute_Bu(self):
        ...
        # Store the non-zero entries in the collocation matrix
        self.Bu = coo_array((np.ones_like(rows, dtype=bool), (rows, cols)), shape=(dofs.shape[0], dofs_u.shape[0])).tocsc()
