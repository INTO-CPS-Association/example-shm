...
def SSI_poles(
    ...
    Ufx_list : np.ndarray, optional
        List of Ufx, returned if `calc_unc` is True.
    """
    ...
    if calc_unc:
        ...
        cov_fx_list = np.full((ordmax, int((ordmax) / step + 1), 2, 2), np.nan)
        Ufx_list = np.full((ordmax, int((ordmax) / step + 1), 2, nb), np.nan)
    ...
    for nn in trange(0, ordmax + 1, step):
        ...
        if calc_unc:
            ...
            for jj in range(len(lam_c)):
                if not np.isnan(fn[jj]):
                    ...
                    Ufx_list[jj,n,:,:] = Ufx #jj = mode in model order, n = model order
        ...
        Phi[: len(fn), n, :] = phi.T
    if calc_unc is True:
        return Fn, Xi, Phi, Lambdas, Fn_std, Xi_std, Phi_std, Ufx_list
    else:
        return Fn, Xi, Phi, Lambdas, None, None, None, None
