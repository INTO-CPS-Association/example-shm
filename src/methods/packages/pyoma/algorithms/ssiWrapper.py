...
from pyoma2.functions import clus, fdd, gen, plot
from pyoma2.support.sel_from_plot import SelFromPlot
import src.methods.packages.pyoma.functions.ssiWrapper as ssi
from src.methods.packages.pyoma.algorithms.baseWrapper import BaseAlgorithm
from src.methods.packages.pyoma.algorithms.data.resultWrapper import ClusteringResult, SSIResult
...
class SSI(BaseAlgorithm[SSIRunParams, SSIMPEParams, SSIResult, typing.Iterable[float]]):
    ...
    def run(self) -> SSIResult:
        ...
        for ii in range(ordmin):
            ...
            nan_Matrix[:,:] = np.nan
            Phis[:,id,:] = nan_Matrix
            Phi_std[:,id,:] = nan_Matrix
            nan_Matrix = np.empty((Ufx_list.shape[0],Ufx_list.shape[2],Ufx_list.shape[3]))
            nan_Matrix[:,:,:] = np.nan
            Ufx_list[:,id,:,:]= nan_Matrix
