...
def dnv_sn(joint_type: str,enviroment: str,**kwargs: Optional[Any])-> dict[str, Any]: #2016 version
    ...
    if joint_type.lower() in ("plate","pile","forged_node","cast_node", "bolt", "pipe"):
        ...
        if (joint_type.lower() == "bolt") and (stress_type == "shear"):
            ...
            try: #Test if a sufficient detail is supplied
                ...
                raise KeyError('Joint must be provided with a detail') from exc
        ...
        #Internal cyclic pressure is not available
            ...
            try: #Test if detail is a pipe detail
                ...
                raise KeyError("Key error") from e
        ...
        try:
            ...
            raise KeyError('Joint must be provided with a surface roughness, R_a [µm]. E.g. surf=3.2') from exc
    ...
    SF  = kwargs.get("SF",1)
    for ii, x in enumerate(Delta_s_R_D):
        Delta_s_R_D[ii] = x*SF*DFF**(-1/3)
    for ii,x in enumerate(C):
        C[ii] = Delta_s_R_D[0]**m[ii]*N_D
        log_a_bar_ = np.log10(C[ii])
