...
import numpy as np
# pylint: disable=C0103, C0301 W3301, R0912, R0915

def bin_func(xx: list[float], hist_data: dict[str, Any],
             **kwargs: Optional[Any]) -> tuple[list[int], list[float]]:
    """Plot SN curve
    ...
    **kwargs (any):
    ...
    def align_hist1D(xx: list[float], hist_prev: list[float], bins_prev: list[float],
                     **kwargs: Optional[Any]) -> tuple[np.ndarray, list[float]]:
        ...
        **kwargs    :   bins, int
        ...
        try:
            ...
            #Find new histogram
            hist_new, bins_new = np.histogram(xx,bins=round(nbins),
                                              range=(round(min(xx)-rem2,2),
                                                round(max(xx)+bin_width-rem1,2))) #Histogram
            if (bins_new[1]-bins_new[0]) != bin_width:
                hist_new, bins_new = np.histogram(xx,bins=round(nbins+1),
                                                  range=(round(min(xx)-rem2,2),
                                                    round(max(xx)+bin_width-rem1,2))) #Histogram
            ...
            nbins = round((max(max(bins_new),max(bins_prev))-min(min(bins_new),
                                        min(bins_prev))) / (bins_new[1]-bins_new[0]) + 1)

            bin_edges = np.linspace(min(min(bins_new),min(bins_prev)),
                            max(max(bins_new),max(bins_prev)),nbins)
            hist = np.add(hist_new,hist_prev) #Add hist counts together

            return hist.astype(dtype=np.int64), bin_edges
        except RuntimeError as exc:
            print("binfunc Debug info", exc)
            print(hist_prev)
            print(hist_new)
            ...
            print(binwidth)
            print(diff)
            raise RuntimeError("hist_new and hist_prev could not be added together") from exc
...
def bin_func2D(xx: list[float], yy: list[float],
               hist_data: dict[str,Any], **kwargs: Optional[Any]
    ...
    **kwargs (any):
    ...
    def align_hist2D(xx: list[float], yy: list[float],
                        ...
                        ybins_prev: list[float], **kwargs: Optional[Any]
                        ) -> tuple[np.ndarray, list[float], list[float]]:
        ...
        **kwargs    :   ybins, int
        ...
        #Find new histrogram
        hist_new, xbins_new, ybins_new = np.histogram2d(xx,yy,
                                        bins=(round(nbins_x), round(nbins_y)),
                                        range=((0,round(max(xx)+bin_width-rem,2)),
                                               (round(min(yy)-rem2,2),
                                                round(max(yy)+bin_width-rem3,2)))) #Histogram
