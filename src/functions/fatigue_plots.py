...
def plot_sn_curve(sn_curve: dict[str, Any], result: dict[str, Any],**kwargs) -> tuple[matplotlib.figure.Figure, dict[str,Any]]:
    """Plot SN curve
    ...
    **kwargs (Any)
        ...
        - figure (matplotlib.figure.Figure): figure to redraw
    ...
    hist_tot, bin_edges = fat.bin_func(stress_list_,**kwargs)

    #For next plot, without residual data
    hist_next, bin_edges_next = fat.bin_func(stress_list_1,**kwargs)
...
def plot_time_series(time_series: list[float],**kwargs: Optional[str]) -> NoReturn:
    """Plot time series

    Args:
        time_series, dict(str, Any): List of time series
        **kwargs (Any)
    ...
    """
    title_string = kwargs.get('title_string',"Time series")
...
def plot_histogram(result: dict[str, Any],**kwargs: Optional[Any]) -> tuple[matplotlib.figure.Figure, dict[str,Any]]:
...
def plot_damage(result: dict[str, Any],**kwargs: Optional[Any]) -> matplotlib.figure.Figure:
...
def plot_cld(sn_curve: dict[str, Any],s_y,s_u,result: dict[str, Any],**kwargs: Optional[Any]) -> matplotlib.figure.Figure:
    ...
    **kwargs:
        title (str): Title to plot
        figure (object): figure to redraw
        result (dict): Dictionary of results from continous data stream
        points (int): Maximum points to display
    
    Returns:
        fig (fig): figure to remember
    """
...
def plot_eol_rul(result: dict[str, Any],inital_time: datetime ,current_time: datetime ,output_time_unit: Optional[str] = "hrs",damage_sum: Optional[float] = 1, x_length: Optional[int] = None,**kwargs: Optional[Any]) -> matplotlib.figure.Figure:
    ...
    **kwargs (Any)
        - figure (fig): figure to update/re-draw
